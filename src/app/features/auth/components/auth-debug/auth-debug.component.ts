...
@Component({
  ...
  template: `
    <div class="fixed top-4 right-4 bg-black text-white p-3 rounded-lg text-sm font-mono z-50">
      ...
      <div><strong>Authenticated:</strong> {{ (isAuthenticated$ | async) ? '✅ YES' : '❌ NO' }}</div>
      <div><strong>Current User:</strong> {{ (currentUser$ | async)?.email || 'None' }}</div>
      <div><strong>Role:</strong> {{ (currentUser$ | async)?.role || 'None' }}</div>
      <div class="mt-2"><strong>Session:</strong> {{ hasSession ? '✅ Found' : '❌ Not found' }}</div>
      <div><strong>Users in DB:</strong> {{ usersCount }}</div>
      <div class="mt-2 space-y-1">
        <button (click)="resetUsers()" class="block w-full bg-red-600 hover:bg-red-700 px-2 py-1 rounded text-xs">
          🔄 Reset Users
        </button>
        <button
          (click)="clearSession()"
